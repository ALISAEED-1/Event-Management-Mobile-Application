...
class _AdminFeaturesState extends State<AdminFeatures> {
  ...
  List<EventModel> get _filteredEvents {
    return _allEvents.where((e) {
      ...
      if (selectedCity != null && !loc.contains(selectedCity!.toLowerCase())) return false;
      if (selectedState != null && !loc.contains(selectedState!.toLowerCase())) return false;
  ...
  Future<void> _toggleFavorite(EventModel event) async {
    ...
    try {
      if (nowFav) {
        await AdminFavoritesService().addFavorite(id);
      } else {
                      ...
                      Expanded(
                        child: OutlinedButton(
                          onPressed: () => setModalState(() {
                            tempCity = null; tempState = null;
                            tempGroup = null; tempCategory = null;
                          }),
                          style: OutlinedButton.styleFrom(
                            side: BorderSide(color: isDark ? Colors.white24 : Colors.grey),
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                          ),
                          child: Text("Clear Filter", style: GoogleFonts.poppins(color: textColor)),
                        ),
                      ),
  ...
  Widget _buildFilterLabel(String text, Color? textColor) => Padding(
    ...
    child: Text(text,
        style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 13, color: textColor)),
  );
  ...
  Widget _buildBody(bool isDark, Color surfaceColor, Color? textColor) {
    ...
    if (_error != null) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.wifi_off_outlined, size: 48, color: Colors.grey.withOpacity(0.5)),
            ...
            TextButton(
              ...
              child: Text('Retry', style: GoogleFonts.poppins(color: const Color(0xFFD32F2F))),
            ),
          ],
        ),
      );
    ...
    if (events.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.event_outlined, size: 64, color: Colors.grey.withOpacity(0.4)),
            const SizedBox(height: 16),
            Text(
              _hasActiveFilter ? 'No events match your filters.' : 'No events yet.',
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) =>
          _buildEventCard(events[index], _favorites.contains(events[index].docId ?? ''), isDark, surfaceColor, textColor),
    );
  }

  Widget _buildEventCard(EventModel event, bool isFav, bool isDark, Color surfaceColor, Color? textColor) {
    return GestureDetector(
      onTap: () => Navigator.push(context,
          MaterialPageRoute(builder: (_) => Event(event: event, initialIsFav: isFav))),
      child: Container(
                      ...
                      Expanded(
                        child: Text(
                          [event.date, event.time]
                              ...
                              .join(' · '),
                          style: GoogleFonts.poppins(
                              fontSize: 13, color: isDark ? Colors.white60 : Colors.black87),
                        ),
                      ),
                    ]),
                  const SizedBox(height: 20),
